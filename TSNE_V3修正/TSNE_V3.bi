...
'--------------------------------------------------------------------------------------------------------------
Private Function TSNE_Create_Server(ByRef R_TSNEID as UInteger, ByRef V_Port as UShort, ByRef V_MaxSimConReq as UShort = 10, ByVal V_Event_NewConPTR as Any Ptr, ByVal V_Event_NewConCancelPTR as Any Ptr = 0, ByVal V_StackSizeOverride as UInteger = TSNE_INT_StackSize, ByVal V_CallbackBackPtr as Any Ptr = 0) as Integer
R_TSNEID = 0
...
#IF DEFINED(TSNE_DEF_REUSER)
	...
	#IF DEFINED(__FB_LINUX__)
		...
		If setsockopt(TSock, SOL_SOCKET, SO_LINGER, @linger_opt, SizeOf(linger_opt)) = -1 then close_(TSock): Return TSNE_Const_CantBindSocket
	#ELSEIF DEFINED(__FB_WIN32__)
...
If BV <> 0 Then
	...
	#IF DEFINED(__FB_LINUX__)
		...
		Dim RV as Integer
		Do
			'RV = connect(TSock, CPtr(SOCKADDR PTR, @TADDR), SizeOf(SOCKADDR))
			'print "RV:" & RV
...
Dim TADDR as SOCKADDR_IN
#IF __FB_VERSION__ >= "1.03"
	Dim XSize as ULong = 16
#ELSE
	Dim XSize as Integer = 16
#ENDIF
Dim OADDR as SOCKADDR_IN 
If getsockname(V_RequestID, Cast(sockaddr PTR, @OADDR), cast(any ptr, @XSize)) = 0 Then R_RemoteShownServerIPA = *inet_ntoa(OADDR.sin_addr)
If getpeername(V_RequestID, Cast(sockaddr PTR, @TADDR), cast(any ptr, @XSize)) = 0 Then R_IPA = *inet_ntoa(TADDR.sin_addr)
Dim TSD as TSNE_Socket Ptr = TSNE_INT_Add()
MutexLock(TSNE_INT_Mutex)
With *TSD
...
'|				If select_(TSock + 1, @TFDSet, 0, 0, @TTV) = -1 Then Exit Do
				...
				If (FD_ISSET(TSock, @TFDSet)) <> 0 Then
					...
					If TEvent.TSNE_NewDataUDP <> 0 Then
						...
						#ENDIF
						#IF DEFINED(TSNE_SERVERSUBCALLBACK)
							TEvent.TSNE_NewDataUDP(TTSNEID, TIPA, T, TEvent.V_AnyPtr)
						#ELSE
...
'						Print "=[" & Str(TTSNEID) & "]CON_REQ"
						...
						#ENDIF
						TIPA = ""
						If getpeername(TNSock, Cast(sockaddr Ptr, @TADDR), cast(any ptr, @XSize)) = 0 Then TIPA = *inet_ntoa(TADDR.sin_addr)
						#IF DEFINED(_TSNE_DODEBUG_)
						...
						Select Case TSD->V_BWL_UseType
							Case 1
								#IF DEFINED(TSNE_SERVERSUBCALLBACK)
									If TSNE_INT_BW_GetPtr(TSD, TIPA) = 0 Then
										MutexUnLock(TSNE_INT_Mutex): If TEvent.TSNE_NewConnection <> 0 Then TEvent.TSNE_NewConnection(TTSNEID, TNSock, TIPA, TEvent.V_AnyPtr)
									Else: MutexUnLock(TSNE_INT_Mutex): close_(TNSock): If TEvent.TSNE_NewConnectionCanceled <> 0 Then TEvent.TSNE_NewConnectionCanceled(TTSNEID, TIPA, TEvent.V_AnyPtr)
									End If
								#ELSE
									If TSNE_INT_BW_GetPtr(TSD, TIPA) = 0 Then
							...
							Case 2
								#IF DEFINED(TSNE_SERVERSUBCALLBACK)
									If TSNE_INT_BW_GetPtr(TSD, TIPA) = 0 Then
										MutexUnLock(TSNE_INT_Mutex): close_(TNSock): If TEvent.TSNE_NewConnectionCanceled <> 0 Then TEvent.TSNE_NewConnectionCanceled(TTSNEID, TIPA, TEvent.V_AnyPtr)
									Else: MutexUnLock(TSNE_INT_Mutex): If TEvent.TSNE_NewConnection <> 0 Then TEvent.TSNE_NewConnection(TTSNEID, TNSock, TIPA, TEvent.V_AnyPtr)
									End If
								#ELSE
									If TSNE_INT_BW_GetPtr(TSD, TIPA) = 0 Then
							...
							Case Else
								MutexUnLock(TSNE_INT_Mutex)
								If TEvent.TSNE_NewConnection <> 0 Then
									#IF DEFINED(TSNE_SERVERSUBCALLBACK)
										TEvent.TSNE_NewConnection(TTSNEID, TNSock, TIPA, TEvent.V_AnyPtr)
									#ELSE
										TEvent.TSNE_NewConnection(TTSNEID, TNSock, TIPA)
									#ENDIF
								End If
						End Select
						#IF DEFINED(_TSNE_DODEBUG_)
